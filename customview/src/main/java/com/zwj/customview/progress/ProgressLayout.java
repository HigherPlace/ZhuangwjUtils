...
/***
 * 用法——当父布局用即可
 *  开启加载： progressLayout.setProgress(true);
 *  关闭加载：progressLayout.setProgress(true);
 * 
...
public class ProgressLayout extends RelativeLayout {
	...
	private void init(AttributeSet attrs) {
		...
		int backgroundColor = a.getColor(
				R.styleable.ProgressLayout_progressLayoutProgressBackground,
				Color.TRANSPARENT);
		boolean startFromProgress = a.getBoolean(
				R.styleable.ProgressLayout_progressLayoutProgress, false);
		a.recycle();

		LayoutParams layoutParams;

		// if progressBackground color == Color.TRANSPARENT just add progress
		// bar
		...
		} else { // else wrap progress bar in LinearLayout and set background
					// color to LinearLayout
